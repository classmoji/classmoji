...
- **API** (`apps/api`): Fastify REST API for core operations
- **Quiz Agent** (`apps/quiz-agent`): Dedicated WebSocket microservice for AI-powered quizzes
...
│   ├── quiz-agent/      # WebSocket quiz microservice
...
The platform uses `assertOrgAccess` helper for unified authorization:
...
webapp/action.server.js
  ↓
WebSocket connection to quiz-agent
  ↓
...
- Quiz agent service: [apps/quiz-agent/](apps/quiz-agent/)
...
PostHog integration for understanding platform usage and student engagement.

#### Analytics Implementation

**Dual Tracking Approach**

- **Client-side** (posthog-js): User interactions, navigation, clicks
- **Server-side** (posthog-node): Data mutations, background events

#### Tracked Events

**Student Events**

- Navigation clicks (assignments, quizzes, grades)
...
**Data Masking** ([posthog.ts](apps/webapp/app/utils/posthog.ts))

- Student names masked in events
- Sensitive PII excluded
- Grades sent as ranges, not exact values
- Email addresses hashed

**Session Recording**

- Enabled with privacy protections
- Form inputs masked
...
- Build funnels (e.g., quiz start → completion)
- Track feature adoption
- Identify usage patterns

#### File Locations

- PostHog setup: [apps/webapp/app/utils/posthog.ts](apps/webapp/app/utils/posthog.ts)
- Client tracking: [apps/webapp/app/root.tsx](apps/webapp/app/root.tsx)
- Server tracking: Throughout action files
- Analytics docs: [apps/docs/ANALYTICS.md](apps/docs/ANALYTICS.md)
...
**Authorization Helper** ([helpers.ts:assertOrgAccess](apps/webapp/app/utils/helpers.ts))
...
async function assertOrgAccess({
  ...
  // 6. Return { userId, organization, membership, isResourceOwner }
}
```

### API Architecture

**Fastify REST API** ([apps/api/](apps/api/))
...
GET    /api/grades/:issueId       Get grades

POST   /api/quizzes               Create quiz
GET    /api/quizzes/:id           Get quiz
POST   /api/quizzes/:id/attempt   Start quiz attempt

GET    /api/tokens/:studentId     Get token balance
...
1. **Admin Route Authentication** (Being addressed)
   ...
   - Solution: `assertOrgAccess` being added to all routes
...
# Individual services
npm run web:dev          # React webapp on :5173
npm run api:dev          # Fastify API on :3000
npm run quiz-agent:dev   # Quiz agent on :8080
...
apps/
  ...
  api/src/                        # Fastify API routes
  quiz-agent/src/                 # Quiz WebSocket service
...
- **Authorization**: `assertOrgAccess()` in `apps/webapp/app/utils/helpers.ts`
