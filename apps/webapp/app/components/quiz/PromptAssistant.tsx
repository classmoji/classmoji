...
interface PromptExplorationStep {
  action: string;
  toolName: string;
}
  ...
  return (
    <div className={`pa-message ${isUser ? 'user' : 'assistant'}`}>
      ...
      <div className="pa-message-content">
        ...
        {((message.suggestions?.length ?? 0) > 0 ? message.suggestions! : suggestions).map(
          (suggestion: PromptSuggestion, idx: number) => (
            <SuggestionCard
              key={idx}
...
function parseSuggestions(text: string): { content: string; suggestions: PromptSuggestion[] } {
