...
const SyllabusBotChat = ({
  ...
  const handleSuggestedQuestion = (question: SuggestedQuestion | string) => {
  ...
  const renderReferences = (references: ContentReference[]) => {
    ...
    return (
      ...
      >
        ...
        <Space wrap>
          {references.map((ref: ContentReference, idx: number) => {
              ...
              >
                <ReactMarkdown
                  ...
                  components={{
                    ...
                    // @ts-expect-error -- react-markdown passes `inline` prop not in standard HTML types
      ...
      {suggestedQuestions.length > 0 && messages.length <= 2 && !isStreaming && (
        ...
        >
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
            {suggestedQuestions.map((q: SuggestedQuestion | string, idx: number) => (
              <button
                key={idx}
        ...
        <button
          ...
          onMouseEnter={e => {
            if (inputValue.trim() && !isStreaming) {
              (e.target as HTMLButtonElement).style.backgroundColor = '#d4a216';
            }
          }}
          onMouseLeave={e => {
            if (inputValue.trim() && !isStreaming) {
