...
const ProfileDropdown = ({ children, placement = 'bottomRight' }: ProfileDropdownProps) => {
  ...
  const MenuItem = ({ label, path }: { label: string; path: string }) => (
    <Link
      to={path}
      className="block w-full text-sm text-ink-1 hover:text-gray-900 dark:hover:text-gray-100 hover:bg-gray-100 dark:hover:bg-neutral-700 rounded-md py-1.5 px-2.5 no-underline transition-colors"
    >
      {label}
    </Link>
  );
    ...
    <div className="w-[220px]">
      ...
      <div className="border-t border-gray-200 dark:border-neutral-700 p-1.5">
        <button
          ...
          className="flex items-center gap-2 w-full text-sm text-ink-1 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-md py-1.5 px-2.5 transition-colors cursor-pointer border-none bg-transparent"
        >
