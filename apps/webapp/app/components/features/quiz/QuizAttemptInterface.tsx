...
/** Exploration step derived from system messages */
...
interface QuizAttemptInterfaceProps {
  ...
  isVisible?: boolean;
  onClose?: (newAttemptId?: string) => void;
  org?: string;
  [key: string]: unknown;
}
...
function QuizAttemptInterface({
  ...
  const [messages, setMessages] = useState<QuizMessage[]>(initialMessages || []);
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      const snapshot = finalizeRef.current ? finalizeRef.current() : null;
      const hasSignificantSession = (snapshot?.totalMs ?? 0) > 1000;
  ...
  useEffect(() => {
    if (initialMessages) {
      ...
      const displayMessages = initialMessages.filter((m: QuizMessage) => m.role !== 'system');
  ...
  useEffect(() => {
    ...
    if (sending && pendingUserMessageRef.current) {
      ...
      if (!dbHasUserMsg) {
        ...
        setMessages([
          ...displayMessages,
          { id: 'pending-user-msg', role: 'USER', content: pendingUserMessageRef.current },
        ]);
      } else {
