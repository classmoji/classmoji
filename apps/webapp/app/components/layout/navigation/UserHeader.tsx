...
const UserHeader = () => {
  ...
  // This header is sticky, so a callout at its default offset would land on
  // top of it. The card is 52px under ~30px of padding (pt-7 at a 17px root
  // font), so its bottom edge sits near 82px: push callouts clear of it for as
  // long as this layout is mounted.
  useEffect(() => {
    const root = document.documentElement;
    root.style.setProperty('--callout-top', '96px');
    return () => {
      root.style.removeProperty('--callout-top');
    };
  }, []);
      ...
      <div className="max-w-[1280px] mx-auto px-4 sm:px-8">
        <div className="flex items-center justify-between h-[52px] px-3 sm:px-6 rounded-2xl bg-panel/80 backdrop-blur-xl ring-1 ring-line shadow-sm">
          <Link
            ...
            className="flex items-center gap-2 no-underline text-[#0d0d10] dark:text-white"
          >
