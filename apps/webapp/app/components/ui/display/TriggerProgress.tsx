...
const TriggerProgress = ({ callback, validIdentifiers, operation }: TriggerProgressProps) => {
  const { fetcher } = useGlobalFetcher();
  const [logs, setLogs] = useState<TriggerRun[]>([]);
  const { close, show, visible } = useDisclosure();
  const completedRunsSet = useRef<Set<string>>(new Set());
...
interface TagsLoaderProps {
  tag: string;
  setLogs: React.Dispatch<React.SetStateAction<TriggerRun[]>>;
  completedRunsSet: React.MutableRefObject<Set<string>>;
  validIdentifiers: string[];
...
const TagsLoader = ({
  ...
  useEffect(() => {
    ...
    if (runs.length > 0) {
      setLogs(runs as unknown as TriggerRun[]);
...
const AllTasksDisplay = ({ logs, isDarkMode }: LogDisplayProps) => {
  // Sort all logs by status priority (errors first, then executing, queued, completed)
  const sortedLogs = useMemo(() => {
    return [...logs].sort((a, b) => {
      const priorityA = (STATUS_PRIORITY as Record<string, number>)[a.status] ?? 5;
      const priorityB = (STATUS_PRIORITY as Record<string, number>)[b.status] ?? 5;
...
const LogDisplay = ({ logs, isDarkMode }: LogDisplayProps) => {
...
const LogLabel = ({ data, isDarkMode }: LogLabelProps) => {
  ...
  const config = (statusConfig as Record<string, { color: string; bg: string }>)[status] || {
    color: THEME.error,
    bg: isDarkMode ? 'rgba(239, 68, 68, 0.2)' : 'rgba(239, 68, 68, 0.1)',
  };
...
const LogContent = ({ data }: { data: TriggerRun }) => {
  const { payload } = data;
