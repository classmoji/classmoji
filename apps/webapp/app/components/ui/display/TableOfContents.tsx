...
interface TableOfContentsProps {
  ...
  onHeadingClick?: (id: string, index: number) => void;
  className?: string;
  position?: 'right' | 'left';
}
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      const isLastHeadingVisible =
        lastHeadingRect.top >= 0 && lastHeadingRect.top <= viewportHeight;
      ...
      if (activeHeading) {
        const id = (activeHeading as HTMLElement).dataset?.blockId || activeHeading.id;
        setActiveId(id);
      }
    };
