...
const MultiSelect = ({ options, defaultValue = [], onSelect, onDeselect }: MultiSelectProps) => {
                ...
                <Checkbox
                  ...
                  onChange={e => {
                    if (e.target.checked) {
                      ...
                      setSelected(selected.filter((s: string) => s !== option.value));
                ...
                {selected.map((s: string) => (
                  <div key={s} className="flex items-center gap-2 text-sm text-gray-800">
                    ...
                    <span className="truncate">
                      {options.find((o: SelectOption) => o.value === s)?.label}
                    </span>
                  </div>
                ))}
              </div>
