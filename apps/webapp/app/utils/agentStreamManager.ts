...
class AgentStreamManager extends EventEmitter {
  sessionOwnership: Map<string, SessionOwnership>;
  private eventBuffers: Map<string, AgentEvent[]>;
   ...
   */
  registerSession(sessionId: string, classroomSlug: string, userId: string) {
   ...
   */
  getSessionOwnership(sessionId: string) {
   ...
   */
  subscribeToSession(sessionId: string, callback: (event: AgentEvent) => void) {
   ...
   */
  subscribeToAttempt(attemptId: string, callback: (event: AgentEvent) => void) {
   ...
   */
  publishEvent(sessionId: string, event: AgentEvent) {
    const eventName = `agent:${sessionId}`;
   ...
   */
  publishStep(sessionId: string, step: Record<string, unknown>) {
   ...
   */
  publishQuizReady(sessionId: string, data: Record<string, unknown>) {
   ...
   */
  publishMessageReady(sessionId: string, data: Record<string, unknown>) {
   ...
   */
  publishAssistantResponse(sessionId: string, data: Record<string, unknown>) {
   ...
   */
  publishContentReference(sessionId: string, data: Record<string, unknown>) {
   ...
   */
  enqueueEvent(sessionId: string, event: AgentEvent) {
   ...
   */
  cleanupSession(sessionId: string, { delayMs = 30000 } = {}) {
   ...
   * @param {string} sessionId
   */
  forceCleanupSession(sessionId: string) {
