...
const RepositoriesTable = ({
  ...
  const [selectedRepos, setSelectedRepos] = useState<Array<{ name: string }>>([]);
  ...
  const handleTableChange = (pagination: { current?: number; pageSize?: number }) => {
    ...
    newParams.set('page', (pagination.current ?? 1).toString());
    newParams.set('pageSize', (pagination.pageSize ?? 25).toString());
  ...
  const rowSelection = {
    selectedRowKeys: selectedRepos.map(r => (r as { name: string }).name),
    onChange: (_: React.Key[], selectedRows: Array<{ name: string }>) => {
    ...
    <div className="min-h-full">
      ...
      <div className="rounded-2xl overflow-hidden bg-panel ring-1 ring-line min-h-[calc(100vh-10rem)] p-5 sm:p-6">
        <Suspense fallback={<RepositoriesTableSkeleton />}>
          <Await resolve={repositoriesPromise} errorElement={null}>
            {resolvedData => (
              <RepositoriesTable
                data={resolvedData}
