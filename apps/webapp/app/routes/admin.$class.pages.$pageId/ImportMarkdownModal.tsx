...
interface ImportMarkdownModalProps {
  open: boolean;
  onCancel: () => void;
  onImport: (data: { markdownFile: File; imageFiles: File[] }) => void;
  isImporting: boolean;
}

function extractImageReferencesClient(markdown: string): ImageReference[] {
  const imageRegex = /!\[([^\]]*)\]\(([^)]+)\)/g;
  const images = [];
  let match;
 ...
 */
async function extractZipContents(file: File | Blob) {
  console.log('[extractZipContents] Loading ZIP...');
  let zip = await JSZip.loadAsync(file);
  let files = Object.values(zip.files);
  console.log(
    '[extractZipContents] ZIP loaded, files:',
    files.map(f => f.name)
  );
  ...
  if (nestedZips.length > 0) {
    ...
    console.log(
      '[extractZipContents] Nested ZIP loaded, files:',
      files.map(f => f.name)
    );
  }

  // Find markdown file (first .md or .txt, prefer .md)
  const mdFiles = files.filter(f => !f.dir && (f.name.endsWith('.md') || f.name.endsWith('.txt')));
  console.log(
    '[extractZipContents] Markdown files found:',
  ...
  const imageEntries = files.filter(f => !f.dir && /\.(png|jpg|jpeg|gif|svg|webp)$/i.test(f.name));
  console.log(
    '[extractZipContents] Image files found:',
    imageEntries.map(f => f.name)
  );
    ...
    onDrop: async droppedFiles => {
      ...
      if (acceptedFiles.length === 0) {
        if (droppedFiles.length > 0) {
          setExtractError(
            'Please drop a .zip, .md, .txt, or image file (.png, .jpg, .gif, .svg, .webp)'
          );
        }
        return;
      }
      ...
      console.log('Detected:', {
        ...
        mdFile: mdFile?.name,
        imageCount: droppedImages.length,
      });
      ...
      if (zipFile) {
        ...
        setExtractError(
          'Please drop a markdown file (.md or .txt) first, or drop it together with images'
        );
      }
    },
  });
    ...
    >
      <div className="space-y-4">
            ...
            {isExtracting ? (
              ...
              <div>
                ...
                <p className="text-xs text-ink-3 mt-2">
                  .zip, .md, .txt, .png, .jpg, .gif, .svg, .webp
                </p>
              </div>
            )}
          </div>
        ...
        {markdownFile && !isImporting && (
          <Alert message="This will replace the current page content" type="info" showIcon />
        )}
          ...
          <Alert
            message={
              <div className="flex items-center gap-3">
                <LoadingOutlined className="text-sky-ink" />
                <div>
