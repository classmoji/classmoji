  ...
  try {
    ...
    if (!(verification as { valid: boolean }).valid) {
      ...
      if (sessionOwnership && sessionOwnership.userId === userId.toString()) {
        console.log(
          `[prompt-assistant-stream] ai-agent verification failed but in-memory ownership valid`
        );
      } else {
        console.warn(
    ...
    if (errMessage?.includes('timeout') || errMessage?.includes('disconnect')) {
      console.warn(
        `[prompt-assistant-stream] ai-agent verification failed, using in-memory fallback`
      );
  ...
  const stream = new ReadableStream({
    ...
    cancel() {
      ...
      if (unsubscribe) {
        unsubscribe();
        console.log(
          `[prompt-assistant-stream] Client disconnected, cleaned up listener for session ${sessionId}`
        );
      } else {
