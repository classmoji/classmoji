...
export const loader = async ({ request }: Route.LoaderArgs) => {
  ...
  if (!query) {
    return Response.json({ error: 'Missing search query parameter (q)' }, { status: 400 });
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return Response.json({ error: 'Unsplash API request failed' }, { status: response.status });
    }
    ...
    return Response.json({ error: 'Failed to search Unsplash' }, { status: 500 });
  }
};
