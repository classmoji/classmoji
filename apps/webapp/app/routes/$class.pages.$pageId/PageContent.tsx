...
export const PageContent = memo(function PageContent({
  ...
  useEffect(() => {
    ...
    headingElements.forEach((heading: Element, index: number) => {
      if (!(heading as HTMLElement).id) {
        const slug = (heading.textContent ?? '')
          ...
          .replace(/[^a-z0-9]+/g, '-')
          .replace(/(^-|-$)/g, '');
        (heading as HTMLElement).id = slug ? `${slug}-${index}` : `heading-${index}`;
      }
    ...
    contentRef.current.querySelectorAll('.code-block').forEach((block: Element) => {
    ...
    contentRef.current.querySelectorAll('.terminal-block').forEach((block: Element) => {
    ...
    contentRef.current.querySelectorAll('pre').forEach((pre: Element) => {
