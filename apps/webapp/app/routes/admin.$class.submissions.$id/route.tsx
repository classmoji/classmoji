...
export const loader = async ({ params, request }: Route.LoaderArgs) => {
  ...
  const studentMemberships = await ClassmojiService.classroomMembership.findStudents(classroom.id);
  ...
  const snapshot = repoAssignment.analytics_snapshot as unknown as GitHubStatsSnapshot | null;
  ...
  return (
    <div className="flex flex-col gap-4" data-testid="submission-analytics">
      <div className="flex items-center gap-2">
        <Link to={backHref}>
          <Button size="small" icon={<IconChevronLeft size={14} />} className="flex items-center">
            Back
          </Button>
        </Link>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">
          {assignmentTitle}{' '}
          <span className="font-mono text-base text-gray-500 dark:text-gray-400">· {repoName}</span>
        </h1>
      </div>

      {refreshError ? <Alert type="error" message={refreshError} closable /> : null}
