...
function generateRandomGrade(assignmentId: number, basePerformance: number) {
...
function generateStudentData(index: number) {
...
export const mockMemberships = mockStudents.map((student, index) => ({
  ...
  user_id: student.id,
  comment:
    index % 5 === 0 ? 'Great work this semester!' : index % 7 === 0 ? 'Needs improvement' : null,
