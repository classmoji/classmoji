...
export function useImageProcessor() {
  ...
  const resizeImage = useCallback(
    async (
      ...
      const { maxWidth = 1920, maxHeight = 1080, quality = 0.85, format = null } = options;
  ...
  const cropImage = useCallback(
    async (
      ...
      const { quality = 0.85, format = null, displayedWidth, displayedHeight } = options;
    ...
    async (
      ...
      if (crop && crop.width && crop.height) {
        ...
        if (processedBlob.size > 0) {
          ...
          processedBlob = await resizeImage(tempFile, {
            maxWidth,
            maxHeight,
            quality,
            format: outputFormat,
