 ...
 */
function getBaseUrl(org: string, repoName: string): string {
 ...
 */
export async function getThemeUrls(org: string, repoName: string, themeName: string) {
 ...
 */
export function generateThemeSlug(displayName: string): string {
  return displayName
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-|-$/g, '')
