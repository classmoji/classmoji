...
class ImportStreamManager extends EventEmitter {
  private eventBuffers: Map<string, ImportEvent[]>;
   ...
   */
  subscribe(slideId: string, callback: (event: ImportEvent) => void): () => void {
   ...
   */
  publish(slideId: string, event: ImportEvent): void {
    const eventName = `import:${slideId}`;
   ...
   */
  scheduleCleanup(slideId: string, delayMs: number = 60000): void {
   ...
   */
  forceCleanup(slideId: string): void {
