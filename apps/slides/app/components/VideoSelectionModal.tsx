...
export default function VideoSelectionModal({
  ...
  useEffect(() => {
    if (open && videos.length > 0) {
      const suggested = new Set<string>(videos.filter(v => v.suggestCloudinary).map(v => v.path));
  ...
  const selectRecommended = useCallback(() => {
    setSelectedPaths(new Set(videos.filter(v => v.suggestCloudinary).map(v => v.path)));
  }, [videos]);
  ...
  const recommendedCount = useMemo(() => videos.filter(v => v.suggestCloudinary).length, [videos]);
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            {recommendedCount > 0 && recommendedCount !== videos.length && (
              <Button size="small" type="primary" ghost onClick={selectRecommended}>
