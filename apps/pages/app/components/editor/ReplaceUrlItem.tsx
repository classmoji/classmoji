 ...
 */
export function ReplaceUrlItem({ children }: { children: React.ReactNode }) {
  ...
  if (!hasUrl || !block.props?.url || !Components) return null;
 ...
 */
export function RemoveProfileImageItem({ children }: { children: React.ReactNode }) {
  const editor = useBlockNoteEditor();
  const Components = useComponentsContext();
  ...
  if (!block) return null;
  if (block.type !== 'profile' || !block.props?.imageUrl || !Components) return null;
