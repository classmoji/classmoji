...
/**
 * Test-only login route that bypasses GitHub OAuth (mirrors the slides app's
 * route; used by the Playwright harness). Creates a Better Auth session
 ...
 *   /test-login?role=owner&redirect=/slug/pageId - Login and redirect
 */
export const loader = async ({ request }: { request: Request }) => {
  // Belt-and-suspenders (webapp pattern): production always blocks, and outside
  // production the explicit ENABLE_TEST_LOGIN flag is required too. Note the
  // pages dev server derives dev mode as NODE_ENV !== 'production' (server.ts),
  // so we gate on that rather than requiring NODE_ENV === 'development'.
  ...
  try {
    ...
    if (!account) {
      ...
      console.log(`[pages/test-login] Token not in DB, fetching from GitHub API...`);
    ...
    console.error('[pages/test-login] Error:', error);
