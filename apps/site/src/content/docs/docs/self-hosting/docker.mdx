...
**Upgrading to the coursework model (assignments in modules, push-mode submissions).** Four migrations carry existing data over: `20260920000000_coursework_phase1_additive`, `20260920000001_coursework_phase1_constraints`, `20260920000002_submission_modes`, and `20260920000003_one_score_per_grader`. They run with `prisma migrate deploy` like any other. Afterwards, run

```bash
npx tsx packages/database/scripts/backfillRepoSubmissions.ts --apply
```

to stamp push-mode submissions from each student repo's push history (run it without `--apply` first for a dry run). The migrations record what they changed in the `coursework_migration_report` table, so you can review moved weights and rewritten records before opening the app to students.
