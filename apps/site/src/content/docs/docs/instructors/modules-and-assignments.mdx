...
After publishing, hit **Sync** if something is missing. A student joined late and doesn't have a repo, or a release date just passed and the issue didn't show up. Sync checks for gaps and fills them in.
...
- **Release date**: when the issue gets created in student repos. Leave blank to release immediately on publish. Useful for drip-releasing work week by week across a semester. Issues are released daily at 12:01 AM, so set the date to the day before if you want something available first thing in the morning.
...
Students work directly in their repo and push their changes. When they're done, they close the issue, which marks the assignment as submitted in Classmoji. Graders can then see it as submitted and grade it.
