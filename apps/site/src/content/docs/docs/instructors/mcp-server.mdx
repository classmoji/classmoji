...
- **Read** — your classrooms, roster, teams and team tags, repositories, assignments and submissions (each with its submission mode and repo URL), the grading queue, leaderboard, regrade requests, calendar, pages, modules, quizzes, forms and their responses, and your token ledger.
- **Act** — add and remove grades, assign graders, resolve regrade requests, create and update assignments in a module (`assignment_create` takes the module, the type, the repository, quiz or form it targets, and for repositories a submission mode, push by default), create, edit and delete repositories (their type, team setup and template lock once student repos exist), create team tags, create and publish modules, create and edit pages — including uploading images into a page and setting one as its cover — manage calendar events, and grant tokens. You can also create and publish forms, label responses as you work through them, and add responses you collected elsewhere — a spreadsheet, a previous term's list — as if the person had filled the form in.
