...
async function cloneRepo(org: string, repoName: string, accessToken: string): Promise<void> {
  const git = simpleGit();
  const remote = `https://x-access-token:${accessToken}@github.com/${org}/${repoName}.git`;
  await git.clone(remote);
}

async function calculateContributions(org: string, repoName: string): Promise<string> {
...
export const calculateContributionsTask = task({
  ...
  run: async (arg: { orgLogin: string; repoName: string; accessToken: string }) => {
