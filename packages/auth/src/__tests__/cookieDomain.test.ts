...
describe('resolveCookieDomain', () => {
  ...
  it('empty and whitespace strings count as unset', () => {
    expect(
      resolveCookieDomain({ COOKIE_DOMAIN: '', SITE_BASE_DOMAIN: 'lvh.me', NODE_ENV: 'development' })
    ).toBe('.lvh.me');
    expect(resolveCookieDomain({ COOKIE_DOMAIN: '  ', SITE_BASE_DOMAIN: '', NODE_ENV: 'test' })).toBeNull();
  });

  it('never doubles the leading dot when SITE_BASE_DOMAIN arrives with one', () => {
    expect(
      resolveCookieDomain({ SITE_BASE_DOMAIN: '.classmoji.io', NODE_ENV: 'production' })
    ).toBe('.classmoji.io');
  });
});
