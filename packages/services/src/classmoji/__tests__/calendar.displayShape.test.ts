...
describe('getClassroomCalendar — the display payload', () => {
  ...
  it('echoes the raw link rows back only when the caller asks for them', async () => {
    ...
    expect(event._rawPageLinks).toEqual([
      { page_id: 'p-pub', occurrence_date: null },
      { page_id: 'p-draft', occurrence_date: null },
    ]);
    expect(event._rawSlideLinks).toEqual([
      { slide_id: 's-pub', occurrence_date: null },
      { slide_id: 's-draft', occurrence_date: null },
    ]);
    expect(event._rawAssignmentLinks).toEqual([
      { assignment_id: 'a-pub', occurrence_date: null },
      { assignment_id: 'a-unpub', occurrence_date: null },
      { assignment_id: 'a-unpub-repo', occurrence_date: null },
    ]);
  });
});
