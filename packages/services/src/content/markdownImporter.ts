...
export async function formatCodeBlocks(markdown: string): Promise<string> {
  ...
  const normalizeLangToken = (token: string | null | undefined): string => {
    if (!token) return '';
  ...
  const guessParser = (langToken: string, code: string): string | null => {
    ...
    if (normalizedToken) {
      ...
      return (parserMap as Record<string, string>)[normalizedToken] || null;
    }
 ...
 */
export function extractImageReferences(markdown: string): Array<{ alt: string; path: string }> {
 ...
 */
export function processToggleLists(markdown: string): string {
 ...
 */
export function processEmojiCallouts(html: string): string {
 ...
 */
export function addHeaderSpacing(html: string): string {
 ...
 */
export function rewriteImagePaths(html: string, imageMap: Map<string, any>): string {
