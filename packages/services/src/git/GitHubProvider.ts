...
const privateKey: string | null = process.env.GITHUB_PRIVATE_KEY_BASE64
...
export class GitHubProvider extends GitProvider {
   ...
   */
  constructor(installationId: string, orgLogin: string | null = null) {
   ...
   */
  async getLatestCommitSHA(org: string, repo: string, branch: string = 'main'): Promise<string> {
    const octokit = await this.#getOctokit();
   ...
   * @returns {Promise<Object[]>}
   */
  async getPendingInvitations(org: string): Promise<any[]> {
    const octokit = await this.#getOctokit();
   ...
   */
  async cancelPendingInvitation(org: string, invitationId: number): Promise<void> {
    const octokit = await this.#getOctokit();
   ...
   * @returns {Promise<Object[]>}
   */
  async getTeams(org: string): Promise<any[]> {
    const octokit = await this.#getOctokit();
