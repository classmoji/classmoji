...
export DATABASE_URL WEBAPP_URL QUIZ_AGENT_URL AI_AGENT_URL SLIDES_URL PAGES_URL
...
cleanup_ports() {
  ...
  if [ "$killed" = true ]; then
    echo "🧹 Cleaned up orphaned processes on ports: $ports"
...
if lsof -i :4000 -sTCP:LISTEN >/dev/null 2>&1; then
  echo "🔀 Webhook fanout already running on port 4000 (started by another session)"
  RUN_FANOUT=false
  PREFIX="$PREFIX_BASE"
else
  echo "🔀 Starting webhook fanout on port 4000"
  RUN_FANOUT=true
...
if [ -f "apps/ai-agent/package.json" ]; then
  ...
  echo "⚠️  ai-agent submodule not initialized — skipping AI agent service"
fi
